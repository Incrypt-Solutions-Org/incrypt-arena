    ...
    <motion.form initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit} className="space-y-6">
      <div className="cyber-card p-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-display text-xl font-bold text-white flex items-center gap-2"><Presentation className="w-5 h-5 text-neon-blue" />Presentation Points</h2>
            <p className="text-sm text-gray-400 mt-1">1st solo: +30 pts, 1st pair: +20 pts • 2nd solo: +20 pts, 2nd pair: +15 pts</p>
          </div>
        </div>
      </div>
      ...
      <div className="cyber-card p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select value={firstPresenter} onChange={(e) => setFirstPresenter(e.target.value)} className="w-full px-4 py-3 bg-cyber-darker border border-gray-700 rounded-lg text-white focus:border-neon-blue focus:outline-none" required>
          ...
          <div>
            ...
            <select value={secondPresenter} onChange={(e) => setSecondPresenter(e.target.value)} className="w-full px-4 py-3 bg-cyber-darker border border-gray-700 rounded-lg text-white focus:border-neon-blue focus:outline-none" disabled={!firstPresenter}>
              <option value="">Solo presentation</option>
              {availableSecondPresenters.map((player) => <option key={player.player_id} value={player.player_id}>{player.player_name}</option>)}
            </select>
          </div>
        </div>
        ...
        {firstPresenter && (
          <div className="p-4 bg-neon-blue/10 rounded-lg border border-neon-blue/30">
            <div className="text-center text-neon-blue font-medium">
              {isSolo ? <span>Solo: +{firstPresenterPoints} points for {players.find(p => p.player_id === firstPresenter)?.player_name}</span> : <span>Pair: +{firstPresenterPoints} for {players.find(p => p.player_id === firstPresenter)?.player_name}, +{secondPresenterPoints} for {players.find(p => p.player_id === secondPresenter)?.player_name}</span>}
            </div>
          </div>
        )}
      ...
      <div className="flex items-center justify-between">
        ...
        <button type="submit" disabled={!topic || !date || !firstPresenter || isSubmitting} className="btn-primary flex items-center gap-2 disabled:opacity-50">
